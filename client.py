   ...
   # Intiailize players
    ...
    while True:
        ...
        if waiting:
            ...
            if(mode == "A"):
                # Get yourHand
                yourHand = s.recv(1024).decode("utf-8")

                # Get their hand
                theirHand = s.recv(1024).decode("utf-8")
        ...
        # Otherwise, if waiting is false
            ...
            if(yourMode == "A"):
                ...
                s.sendall(bytes(handToAttack, "utf-8"))

                # Get hand to attack with
                yourHand = input("Hand to attack with? (L/R)")
                s.sendall(bytes(yourHand, "utf-8"))
            ...
            # If mode is "S"
                ...
                s.send(bytes(handToSplit, "utf-8"))
